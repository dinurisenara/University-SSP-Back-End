...
exports.setAssessmentGrade = async (req, res) => {
    ...
    try {
        ...
        let existingGrade = await AssessmentGrades.findOne({ assessmentId, studentId });
        ...
        if (updatedModuleGrade) {
            res.status(200).json({ newAssessmentGrade, updatedModuleGrade });
        } else {
...
async function updateModuleGrades(assessmentMark, weightage, studentId, moduleId, previousGrade) {
    try {
        ...
        if (!moduleGrade || !gradeDocument) {
            console.log('Module grade or grade document not found.');
            return null;
        }
        if (previousGrade) {
        ...
        gradeDocument.gradings.find(grading => grading.moduleId.toString() === moduleId.toString()).grade = moduleGrade.grade;
...
async function getModuleGrade(studentId, moduleId) {
    try {
        ...
        let gradeDocument = await Grade.findOne({ studentId: studentId });
        ...
        if (gradeDocument) {
            ...
            if (moduleGrade)
                ...
                gradeDocument.gradings.push(newModuleGrade);
                await gradeDocument.save();
                return { gradeDocument, moduleGrade: newModuleGrade };
            }
        }
        else {
